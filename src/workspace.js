...
export function ensureSchedulerDesign(p){ const scheduler=path.join(p.automations,'scheduler-design.json'); if(!fs.existsSync(scheduler)) fs.writeFileSync(scheduler, JSON.stringify({version:2,note:'Enable only the local automations you want. External effects require separately configured connectors.',jobs:[{id:'weekly-review',command:'jobos review weekly --profile <profile> --output markdown',schedule:'0 9 * * 5',externalSideEffects:false},{id:'daily-discovery',command:'jobos daily --profile <profile> --json',schedule:'0 7 * * 1-5',externalSideEffects:false}],policy:{autoApply:'disabled',autoSend:'disabled',configuredExternalActions:'user_configured'}},null,2)+'\n'); }
