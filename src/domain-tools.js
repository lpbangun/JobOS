...
export const DOMAIN_TOOLS = Object.freeze([
  ...
  { name: 'tailor_resume', description: 'Create an evidence-grounded tailored resume draft with optional exact-revision PDF and DOCX rendering.', inputSchema: required({ jobId: text, profileId: text, templateId: { type: 'string', enum: ['classic', 'modern', 'executive', 'technical'] }, accentColor: text, layoutProfileId: { type: 'string', enum: ['professional', 'technical', 'leadership'] }, pageSize: { type: 'string', enum: ['letter', 'a4'] }, pageLimit: { type: 'number' }, density: { type: 'string', enum: ['compact', 'standard', 'spacious'] }, format: { type: 'string', enum: ['markdown', 'pdf', 'docx', 'both'] }, sectionOrder: { type: 'array', items: { type: 'string' } } }, ['jobId', 'profileId']) },
...
export async function callDomainTool(s, name, args = {}, options = {}) {
  ...
  if (name === 'tailor_resume') return await tailor(s, args.jobId, args.profileId, 'resume', { templateId: args.templateId, accentColor: args.accentColor, layoutProfileId: args.layoutProfileId, pageSize: args.pageSize, pageLimit: args.pageLimit, density: args.density, format: args.format, sectionOrder: args.sectionOrder });
