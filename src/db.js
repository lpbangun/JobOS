...
import { mkdirs, ensureSchedulerDesign } from './workspace.js';
...
export async function openStore(flags={}){
  ...
  db.run(schema); migrate(db); db.run('INSERT OR REPLACE INTO meta VALUES (?,?)',['schema_version','2']);
  ensureSchedulerDesign(p);
  const store={db,p,root:r}; save(store); return store;
}
export function reload(s){ if(!fs.existsSync(s.p.db)) return s; try { s.db.close(); } catch {} s.db = new SQL.Database(fs.readFileSync(s.p.db)); s.db.run(schema); migrate(s.db); return s; }
