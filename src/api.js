...
export async function handleApi(s,req,res,u){
  try{
    ...
    if(req.method==='PATCH' && resource==='artifacts' && rid){ if(!one(s,'SELECT id FROM artifacts WHERE id=?',[rid])) return send(res,404,{error:'not found'}); const note=data.note||data.notes||''; const result=reviewArtifact(s,{artifactId:rid,approvalStatus:data.approvalStatus||data.approval_status||data.status,note,source:'api'}); return send(res,200,result); }
