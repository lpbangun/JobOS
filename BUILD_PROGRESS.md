...
- `jobos setup`, `setup status`, and `setup next` expose a resumable projection over seven local core steps. First-run TUI setup opens as a centered, focused workspace on the first actionable task, advances atomically after successful actions, supports arrows, optional `j`/`k`, Tab/Shift+Tab, complete click-as-Enter mouse actions, and direct `1`–`7` navigation, and keeps profile/job ambiguity explicit. File browsing opens at Home or the remembered directory with quick access to Home, Documents, Downloads, and the working directory; blocked help names the missing prerequisite and recovery. Resume intake supports paste, file browsing, path validation, extraction preview, and immediate claim review; job intake supports pasted descriptions, URLs, local files, and discovery configuration.
- `jobos tui --profile <id>` opens the primary responsive terminal dashboard with real SQLite data, aligned job/detail geometry, a widened information-first job list, one shared recommended action across priority/readiness/setup surfaces, an on-demand assistant workspace, useful empty states, and technical diagnostics disclosed only with `e`.
...
- Data-bound terminal shell: filled visual hierarchy, single priority strip, responsive job list, decision-first selected detail, collapsed unfocused assistant space, progressive technical details, human-gated review/discovery/stage actions, responsive artifact documents, complete keyboard/mouse parity for actionable rows, incremental line rendering, and clean raw-terminal shutdown.
...
## Verification
- TUI usability verification on 2026-08-03: `npm test` passed **640/640** checks; `npm run smoke` passed the clean-workspace W03/W04/W07/W08 workflow with zero external side effects.
- The advisor-owned usability suite converged in two iterations and passed **16/16** focused benchmarks plus **10/10** guided-navigation checks across `60×20`, `80×24`, `120×36`, and `160×50` layouts. All ten requested criteria scored **10/10**: setup stability, compact source guidance, blocker recovery, intentional empty states, decision-first hierarchy, priority density, minimum-size navigation, honest optional states, one recommended action, and aligned geometry. Additional gates cover responsive information priority, no-ellipsis copy, click-as-Enter behavior, Home/recent file browsing, atomic profile advancement, and progressive evidence disclosure.
