...
- `jobos network paths|contacts --job <id>` retains job-scoped path planning. `network opportunities|graph|health --profile <id>` exposes deterministic profile-level warmth and bounded two-hop paths; trusted `network record` updates contact recency locally.
...
- Disabled-by-default `profile_network_research` and `network_nurture` scheduler actions reuse the existing research and scheduler runtimes. Cold strategic relationships create local nurture tasks and `draft_needs_human_review` check-in artifacts only.
...
- Data-bound terminal shell: filled visual hierarchy, actionable-only priority navigation, responsive job list, decision-first selected detail, compact minimum-size actions, useful wide-terminal context, explicitly focused and scrollable technical details with lossless wrapping, collapsed unfocused assistant space, human-gated review/discovery/stage actions, responsive artifact documents, keyboard/mouse focus parity, incremental line rendering, and clean raw-terminal shutdown.
