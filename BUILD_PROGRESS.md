...
- `jobos setup`, `setup status`, and `setup next` expose a resumable projection over seven local core steps. First-run TUI setup opens as a centered, focused workspace on the first actionable task, advances atomically after successful actions, supports arrows, optional `j`/`k`, Tab/Shift+Tab, isolated mouse reports, and direct `1`–`7` navigation, and keeps profile/job ambiguity explicit. Long file lists scroll with their highlighted row; blocked help names the missing prerequisite and recovery. Resume intake supports paste, file browsing, path validation, extraction preview, and immediate claim review; job intake supports pasted descriptions, URLs, local files, and discovery configuration.
- `jobos tui --profile <id>` opens the locked 011 pipeline/list/detail/agent shell with real SQLite data, a single priority hierarchy, decision-first selected-job context, useful empty states and agent prompts, responsive overlays, direct domain actions, and a default-on Hermes ACP guest.
...
## Verification
- Setup-navigation verification on 2026-08-03: `npm test` passed **632/632** checks; `npm run smoke` passed the clean-workspace workflow with W03/W04/W07/W08 projections and zero external side effects. A real PTY drill opened guided setup, selected the exact resume row with one SGR mouse click, and exited cleanly with `Q`.
- The advisor-owned usability suite converged in four iterations and passed **8/8** focused benchmarks covering `60×20`, `80×24`, `120×36`, and `160×50` layouts; source-copy completeness; blocker/recovery help; empty and populated dashboard hierarchy; priority-strip density; minimum-size navigation; and honest optional-step states. The focused guided-navigation suite additionally passed **10/10** interaction and atomic-render checks.
