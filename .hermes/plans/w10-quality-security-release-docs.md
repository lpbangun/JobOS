...
| `packet-bound-browser-outcome` | W02 inspect → approved readiness → packet → manual/adapter outcome path: public result binds packet ID/hash/fingerprint/receipt state, never answer values, URL query secrets, browser state, cookie, locator secret, or confirmation secret. Confirm MCP/ACP retains inspection-only human-gate policy. |
...
3. A tracked-file guard rejects `.jobos/`, `jobos-workspace/`, `*.sqlite`, `*.sqlite-*`, `.env`, `.env.*` (except documented safe examples if ever added), browser storage/cookie exports, and `release-evidence/` paths in `git ls-files`. It must also ensure `.gitignore` contains the runtime state patterns.
4. A generated-data leak fixture writes unique sentinels to private Career Memory notes/payload, restricted answer values, URL query/userinfo, fake cookie/browser state, MCP/ACP stderr/transcript input, and form locator/confirmation input. It invokes real safe projections/CLI/MCP paths and scans JSON/text/YAML/audit outputs and release artifacts for exact sentinels. The test must prove no secret-bearing raw database, browser state, or runtime mirror is copied into golden/release output. This is a containment test, not a regex-only source scan.
...
**Step 1 — RED:** Add `tests/w10-golden.test.js` with one `test()` per table family above. Initially load a nonexistent manifest entry and assert the intended canonical expected projection so every test fails specifically with `W10 fixture missing`.
...
**Step 3 — GREEN:** Create `manifest.json`, fixture JSON files, and the smallest helper that: creates real temporary workspaces; copies only existing seed fixtures; injects fixed `asOf`; calls existing services/CLI/MCP framing; canonicalizes JSON recursively with sorted object keys; and compares exact safe public projections. Name each fixture with its contract family and source W-wave.
...
**Objective:** Make W08’s most release-sensitive trust boundaries independently replayable with real storage and history.
...
**Step 1 — RED:** Write real tests for (a) generated sentinel containment across SQL.js/audit/mirror/CLI/MCP/ACP/release projection outputs, (b) tracked-file/runtime ignore guard, (c) static raw-secret output guard with narrow allowlist, (d) deterministic release-evidence schema and no-deploy command allowlist, and (e) audit-result classification. Begin with nonexistent `scripts/check-release-security.js` / `release:evidence` and assert their required JSON contracts.
...
**Step 3 — GREEN:** Implement `check-release-security.js` as a read-only checker with `--format json --output <ignored-dir>` that uses `git ls-files`, `.gitignore`, controlled temporary fixture generation, and explicit sentinel scans. Add npm scripts:
```json
...
```
Implement `scripts/release-evidence.js` only after its test is red: it invokes focused commands with `spawnSync` argument arrays, writes all output below its ignored output root, captures audit JSON/nonzero/network-inconclusive outcome, and exits nonzero if any required gate is not `passed`. It must use a subprocess cwd temporary workspace for smoke and never initialize a repository-root `.jobos/` or mirror.
...
**Step 1 — RED:** Add docs-status tests that parse source documents and assert: README describes packet v2/receipt states and states human-gate/MCP inspection-only boundaries; README has a Career Memory section that names provenance, accepted-only effect, revoke/undo, profile isolation, and private-note exclusion; BUILD_PROGRESS no longer claims stale W10-incompatible MCP tool count/test count as current; docs distinguish intentional deferred work from shipped packet/receipt/W08/W09 behavior; no doc promises an MCP SDK migration. Assert workflow uses Node 22, `npm ci --ignore-scripts`, `npm run test:w10`, `npm run release:evidence`, smoke, and uploads only redacted/public release evidence on failure/success—no deploy/publish/tag/push action.
...
**Step 3 — GREEN:** Update `quality.yml` with ordered jobs/steps: clean dependency install; focused W10 goldens/security/docs checks; full existing test suite; isolated smoke; release evidence; artifact upload from `.tmp/release-evidence` only. Keep pull-request/push-to-main triggers and do not add credentials or deployment permissions. Correct README and BUILD_PROGRESS using current verified facts; add a concise W10 verification entry that links commands rather than claiming a future test count. Document release invocation, artifact location, audit policy, and no-deploy guarantee.
...
**Step 1 — RED:** Add a real-client compatibility test requiring `scripts/mcp-demo.js --json --output <temp>` to record requested/offered protocol version, initialize/list/call result, framing, catalog count/names, typed errors, and zero leaked sentinel. Test the current supported `2024-11-05` client path; no SDK dependency is introduced in RED.
...
**Step 6 — commit:**
```bash
git add docs/mcp-compatibility-decision.md scripts/mcp-demo.js tests/w10-mcp-compatibility.test.js tests/mcp-framing.test.js
...
npm ci --ignore-scripts
npm run test:w10
node --test --test-concurrency=1 tests/w10-docs-status.test.js tests/w10-mcp-compatibility.test.js
npm test
npm run smoke
...
**Expected creates:** `tests/fixtures/w10/*.json`, `tests/w10-golden.test.js`, `tests/w10-security-release.test.js`, `tests/w10-docs-status.test.js`, `tests/w10-mcp-compatibility.test.js`, `scripts/check-release-security.js`, `scripts/release-evidence.js`, `docs/mcp-compatibility-decision.md`.
